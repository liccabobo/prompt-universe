...
3. 讀 `illustration/seeds/` 相似 seed（可用 `select-seeds.lisp`）
...
複製 `output/lisp-drafts/TEMPLATE.lisp`，或參考 `illustration/seeds/prop-design/GOLDEN-MANGO-JELLY.md` 的 section 層級。
...
變體 authoring 必須留在 Lisp DSL 內。不要為了產生大型 `defexpansion-plan` 新增 Python / shell / JavaScript 等外部 codegen script；如果需要程式化產生 mutations，請在 Lisp runtime 內用資料、函式或 macro 實作。
...
**禁止**：修改 `illustration/seeds/`、跳過 Lisp 直接寫最終咒語、未確認就 batch、未明確指示就 publish。
