...
`illustration/seeds/` 是 curated seed library，不是普通草稿區。
...
`illustration/seeds/` 下的所有咒語都禁止由 AI 修改。這些檔案只能作為 immutable seeds 被讀取、分析、檢索、審核與引用。若 validator 發現問題，必須輸出 report，不得直接改 source。
...
AI + Lisp macro / compiler 產生的咒語不放入 `illustration/seeds/`。
...
`output/approved/` 仍然不是 `illustration/seeds/`。是否把 approved prompt 升格為 source seed，只能由使用者決定。
...
5. 使用第一個 source prompt：`illustration/seeds/prop-design/GOLDEN-MANGO-JELLY.md`，作為測試案例而不是架構中心。
...
9. 對 `illustration/seeds/` 跑第一份 responsible audit，輸出 `output/reports/source-responsible-audit.md`，只報告不修改 source。
