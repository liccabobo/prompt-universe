...
((:variant-count . 10)        ; 變體數量：本次批次或 evolve run 要生成的 prompt 數（預設 10）。
 ...
 (:evolution-fusion . 75)     ; 演化融合強度：控制多個 evolution / rule 混合時的融合程度。
 (:material-accent . "prism") ; 材質強調：追加的材質語義；空字串 "" 表示不套用。
 (:leap-creativity . nil)     ; 創意跳躍：:allow 允許跨抽象層變化，nil 限制在同層級替換。
 (:axis-threshold . 50))      ; 軸驗證門檻：滑桿值達此門檻時，該變化軸必須通過驗證。
